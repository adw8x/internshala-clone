...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    let code: string;
    try {
      code = await createEmailOtp(String(email), LANG_FR_PURPOSE);
    } catch (err) {
    ...
    try {
      await sendEmail({
        to: String(email),
        subject: "Your Language Verification Code - Internshala Clone",
        text: `Your OTP to switch to French is: ${code}\nIt expires in 5 minutes. Do not share this with anyone.\n\n- Internshala Clone`,
        html: `
          <div style="font-family:Arial,sans-serif;max-width:480px;margin:auto;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">
            <div style="background:#2563eb;color:#fff;padding:20px 28px">
              <h1 style="margin:0;font-size:18px">Internshala Clone - Language Verification</h1>
            </div>
            <div style="padding:24px 28px;text-align:center">
              <p style="color:#374151;font-size:14px">Use the OTP below to verify your email before switching to French. It expires in <strong>5 minutes</strong>.</p>
      ...
      console.error("Language OTP email failure:", mailErr);
